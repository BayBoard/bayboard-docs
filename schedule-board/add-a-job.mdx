...
Scheduled Time is the block length in hours, not a clock time, in half-hour steps. Promised By Date and Promised By Time must both be set, or both left empty.

### RO Details

| Field | Notes |
| --- | --- |
| RO #* | Digits, or PENDING. Editable after save unless the job status is Completed. |
| Job Type* | Required. |
| First Name* | Required. |
| Last Name* | Last Name searches customers after 2 characters. |
...
- **"RO # must contain digits or PENDING."** Use numbers only, or type the word PENDING in the field.
...
- **"Phone must contain 10 to 15 digits."** Remove letters and check that the phone has 10 to 15 digits in it.
- **"Vehicle make or model is required."** Fill in at least one.
- **"Promised By date and time must both be selected."** Set both fields, or clear both promise fields together.
- **"The scheduled block must fit within the shop's open hours."** Pick an earlier start or a shorter scheduled block.
- **"Status must be Assigned when a Technician and Tech Start Time are selected."** Set Status to Assigned, or clear the Technician and start time.
- **"Status must be In Queue until both a Technician and Tech Start Time are selected."** Set Status to In Queue, or finish placing the job on a technician.
