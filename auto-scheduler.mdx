...
description: Place Unassigned jobs into open technician slots.
---
...
- Skill match. The technician can do this job type. A technician's "avoid" list excludes them unless the job is tagged "For" that technician in Unassigned.
...
- Respect time off. Jobs never land in an all-day or partial-day time-off block on a tech.
- No double-booking. Two jobs cannot share a slot on the same technician at once.
- Fits before end of day. A block cannot run past closing.
...
<CardGroup cols={2}>
  ...
  <Card title="Forward and reset" href="/schedule-board/carry-forward-and-reset-day">
