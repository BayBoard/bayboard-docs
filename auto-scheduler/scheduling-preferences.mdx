...
description: Rules and priorities Auto-Schedule follows.
---
...
The screen opens with a short explanation titled "How Auto-Schedule decides": "Rules-based and deterministic. The same board, Unassigned list, and settings always produce the same schedule. Hard constraints are checked first; soft preferences run in the order below, top wins. Order is not a tiebreak: a higher preference counts for much more, so it usually decides which tech gets the job."
...
- **Skill match** (toggle). Tech can do this job type. Their "avoid" list excludes them from it.
- **Parts received** (toggle). Waiting-on-Parts jobs only place when the parts ETA is today or has already passed.
- **Respect time off** (Locked). Never place a job during an all-day or partial-day time-off block on that technician column.
- **No double-booking** (Locked). Two jobs can't share one slot.
- **Fits before end of day** (Locked). The scheduled block must end by that weekday's shop closing time on the grid.
