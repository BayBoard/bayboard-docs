...
description: Set Auto-Schedule rules in Settings > Scheduling.
---
...
Hard constraints are always enforced first. Locked constraints cannot be turned off because they protect the board from invalid placements.

- **Skill match** (toggle). Tech can do this job type. Their "avoid" list excludes them from it.
- **Parts received** (toggle). Waiting-on-Parts jobs only place when parts ETA is today or earlier than today.
- **Respect time off** (locked). Never place a job during an all-day or partial-day time-off block on the column.
- **No double-booking** (locked). Two jobs can't occupy the same slot on the same tech.
- **Fits before end of day** (locked). The scheduled block must end by that weekday's shop closing time.
